...
.DESCRIPTION
  - Discovers monthly pages from the official SCC listing and downloads the "Full version" file.
  - Pulls the MA Plan Directory ZIP and maps Contract -> Parent Organization.
  - Outputs tidy CSVs under data/processed/.
  - Defaults to 24 months; change with -RollingMonths.
  - Excludes AK, HI, DC, and territories.

.SOURCES
  - SCC listing + monthly pages (Full download): CMS Monthly MA Enrollment by State/County/Contract.  (Source link scraped)  # citeturn4search36
  - Parent Org: CMS MA Plan Directory ZIP.  # citeturn4search33
  - Publication cadence: monthly updates by the 15th.  # citeturn2search1
#>
...
function Get-SccFullDownloadUrl {
  ...
  # Try to find link near the phrase "Full version"
  $idx = $html.IndexOf("Full version", [System.StringComparison]::OrdinalIgnoreCase)
  if ($idx -ge 0) {
    $start = [Math]::Max(0, $idx - 1000)
    $length = [Math]::Min(2000, $html.Length - $start)
    $snippet = $html.Substring($start, $length)
    $hrefs = Get-AllHrefs $snippet
    foreach ($u in $hrefs) {
      if ($u -match '\\.(csv|zip|xlsx|xls)(\?|$)') { return (Get-AbsoluteUrl $u) }
    }
  }
  # Fallback: any data file on the page
  $hrefs2 = Get-AllHrefs $html
  foreach ($u in $hrefs2) {
    if ($u -match '\\.(csv|zip|xlsx|xls)(\?|$)') { return (Get-AbsoluteUrl $u) }
  }
  throw "No data download found on $MonthPageUrl"
}
...
function Normalize-SccFrame {
  ...
  function pick { param([string[]]$alts)
    foreach ($a in $alts) {
      if ($lower.ContainsKey($a)) { return $lower[$a] }
      foreach ($k in $lower.Keys) {
        if ($k.Replace(" ","") -like "*$( $a.Replace(' ','') )*") { return $lower[$k] }
      }
    }
    return $null
  }
...
function Join-Object {
  param(
    [Parameter(Mandatory=$true, ValueFromPipeline=$true)] $Left,
    [Parameter(Mandatory=$true)] $Right,
    [hashtable]$On,
    [ValidateSet('Inner','LeftOuter','RightOuter','Full')]
    [string]$Kind = 'Inner',
    [string[]]$Property
  )
  begin {
    $leftList  = New-Object System.Collections.Generic.List[object]
    $rightList = @{}
    foreach ($r in $Right) {
      $key = ($On.Keys | ForEach-Object { $r.( $On[$_] ) }) -join "||"
      if (-not $rightList.ContainsKey($key)) { $rightList[$key] = @() }
      $rightList[$key] += ,$r
    }
  }
  process { $leftList.Add($Left) }
    ...
    foreach ($l in $leftList) {
      $key = ($On.Keys | ForEach-Object { $l.( $_ ) }) -join "||"
      $matched = $rightList[$key]
      if ($matched) {
        foreach ($r in $matched) {
          if ($Property) {
            $o = New-Object psobject
            foreach ($p in $Property) {
              $o | Add-Member NoteProperty $p $( if ($l.PSObject.Properties[$p]) { $l.$p } elseif ($r.PSObject.Properties[$p]) { $r.$p } else { $null } )
            }
            $o
          } else {
            [pscustomobject]@{ Left=$l; Right=$r }
          }
        }
      } elseif ($Kind -in 'LeftOuter','Full') {
        if ($Property) {
          $o = New-Object psobject
          foreach ($p in $Property) {
            $o | Add-Member NoteProperty $p $( if ($l.PSObject.Properties[$p]) { $l.$p } else { $null } )
          }
          $o
        } else {
          [pscustomobject]@{ Left=$l; Right=$null }
        }
      }
    }
    if ($Kind -in 'RightOuter','Full') {
      $leftKeys = $leftList | ForEach-Object { ($On.Keys | ForEach-Object { $_ = $_; $PSItem.( $_ ) }) -join "||" }
      foreach ($kv in $rightList.GetEnumerator()) {
        if ($leftKeys -notcontains $kv.Key) {
          foreach ($r in $kv.Value) {
            if ($Property) {
              $o = New-Object psobject
              foreach ($p in $Property) {
                $o | Add-Member NoteProperty $p $( if ($r.PSObject.Properties[$p]) { $r.$p } else { $null } )
              }
              $o
            } else {
              [pscustomobject]@{ Left=$null; Right=$r }
            }
          }
        }
      }
...
$target = $months | Where-Object {
  $p = [datetime]::ParseExact($_.Key, "yyyy-MM", $null)
  $p -ge $cutoffDt
}
...
foreach ($m in $target) {
  ...
  try {
    ...
    if ($ext -eq ".zip") {
      $rows = Import-FromZipFirstCsv -ZipPath $rawPath
    } elseif ($ext -eq ".csv") {
      $rows = Import-Csv -LiteralPath $rawPath
    } else {
      Write-Warning "Skipping $period; unsupported file type: $ext"
      continue
    }

    $frame = Normalize-SccFrame -Rows $rows -Period $period
    $frames.Add($frame)
    Write-Host ("Loaded SCC {0}: {1} rows" -f $period, $frame.Count)
  }
  catch {
    Write-Warning "Skipping $period due to error: $($_.Exception.Message)"
  }
}
...
$scc = $scc | Where-Object {
  $okByName = $true
  if ($_.state -and $excludeNames -contains $_.state) { $okByName = $false }
  $okByAbbr = $true
  if ($_.state_abbr -and $excludeAbbr -contains $_.state_abbr) { $okByAbbr = $false }
  $okByName -and $okByAbbr
}

# Attach Parent Organization (from MA Plan Directory)
Write-Host "Fetching Parent Organization map from MA Plan Directory..." -ForegroundColor Cyan
try {
  $parentMap = Get-ParentOrgMap -PlanDirUrl $PlanDirectoryUrl -PeriodHint $latestPeriod
  $parentLut = @{}
  foreach ($r in $parentMap) { $parentLut[$r.contract_id] = $r }

  $scc = $scc | ForEach-Object {
    $x = $_
    $po = $null; $on = $null
    if ($parentLut.ContainsKey($x.contract_id)) {
      $po = $parentLut[$x.contract_id].parent_org
      $on = $parentLut[$x.contract_id].org_name_dir
    }
    [pscustomobject]@{
      ...
      org_name      = $(if ($x.org_name) { $x.org_name } elseif ($on) { $on } else { $null })
      parent_org    = $(if ($po) { $po } else { $x.org_name })
      plan_id       = $x.plan_id
  ...
  Write-Warning "Parent Org enrichment failed: $($_.Exception.Message). Continuing with org_name only."
  $scc = $scc | ForEach-Object {
    $_ | Add-Member -NotePropertyName parent_org -NotePropertyValue $_.org_name -Force
    $_
  }
}
...
# ---------------------------
# (Optional) County-level KPIs: current, MoM, YoY + Top Parent Orgs
# ---------------------------
try {
  $byMonth = $scc | Group-Object state, county, report_period | ForEach-Object {
    $state,$county,$rp = $_.Name -split ', '
    [pscustomobject]@{
      state         = $state
      county        = $county
      report_period = [datetime]$rp
      enrollment    = ($_.Group | Measure-Object -Property enrollment -Sum).Sum
    }
  }

  $latestByCounty = $byMonth | Group-Object state, county | ForEach-Object {
    $rows = $_.Group | Sort-Object report_period
    if ($rows.Count -eq 0) { return }
    $last  = $rows[-1]
    $prev  = $(if ($rows.Count -ge 2) { $rows[-2] } else { $null })
    $yoy   = $rows | Where-Object { $_.report_period -eq $last.report_period.AddYears(-1) } | Select-Object -First 1
    ...
    [pscustomobject]@{
      state             = $last.state
      county            = $last.county
      latest_period     = $last.report_period.ToString("yyyy-MM")
      latest_enrollment = [int]$last.enrollment
      mom_change        = $(if ($prev) { fmtDelta $last.enrollment $prev.enrollment } else { $null })
      yoy_change        = $(if ($yoy)  { fmtDelta $last.enrollment $yoy.enrollment } else { $null })
    }
  }

  $latestMonth = ($scc | Measure-Object -Property report_period -Maximum).Maximum
  $tops = $scc | Where-Object { $_.report_period -eq $latestMonth } |
          Group-Object state, county, parent_org |
          ForEach-Object {
            $state,$county,$parent = $_.Name -split ', '
            [pscustomobject]@{
              state       = $state
              county      = $county
              parent_org  = $parent
          ...
          Group-Object state, county |
          ForEach-Object {
            $_.Group | Sort-Object enrollment -Descending | Select-Object -First 5
          }

  $topsAgg = $tops | Group-Object state, county | ForEach-Object {
    $state,$county = $_.Name -split ', '
    $list = ($_.Group | Sort-Object enrollment -Descending |
            ForEach-Object { "{0} ({1:N0})" -f $_.parent_org, $_.enrollment }) -join "; "
    [pscustomobject]@{ state=$state; county=$county; top_parent_orgs=$list }
  }
  ...
  Write-Warning "KPI calculation failed: $($_.Exception.Message)"
}
